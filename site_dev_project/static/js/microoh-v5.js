    ...
    $('#search').on({
        ...
        keyup: function () {
            ...
            if (keyWord) {
                ...
                $.get('/search/', {'word': keyWord}, function (dat) {
                    var str = ''
                     console.info(dat)
                    $.each($.parseJSON(dat), function (k, v) {

                        str += '  <a href="careercourse/course/" name="course" >'+v.fields.name+'</a>';
                    })
                    console.info(str)
                    if (str != '') {
                        $("#keyword-group").find("[name='profession']").html(str)
